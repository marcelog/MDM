/*!
 * \file mdm_test_device_dslam_alcatel_73xx.c A test for dslams alcatel 73xx.
 *
 * # ./mdm_test_device_dslam_alcatel_73xx list
 *
...
#define	LOGFULL		1
#define	LOGPATH		"/tmp/mdm_test_device_dslam_alcatel_73xx.log"
...
main(int argc, char *argv[])
{
	...
	mdm_device_new(&device, MDM_DEVICE_DSLAM_ALCATEL_73xx, &status);
	...
	if(strcmp(argv[1], "list") == 0)
	{
		...
		while((
			cmd_name = mdm_devicecmd_2stringname(MDM_DEVICE_DSLAM_ALCATEL_73xx, i
		)) != NULL)
		{
			fprintf(
				stdout, "(%.2d) - !!%s!!%s!!\n",
				i, cmd_name, mdm_devicecmd_2string(MDM_DEVICE_DSLAM_ALCATEL_73xx, i)
			);
			i++;
		}	
		goto done;
		...
		if(!mdm_devicecmd_isvalid(MDM_DEVICE_DSLAM_ALCATEL_73xx, cmd))
		{
		...
		mdm_device_open(
			...
			sizeof(mdm_device_dslam_alcatel_73xx_options_t), 
			&status
		);
		if(status.status == MDM_OP_ERROR)
		{
