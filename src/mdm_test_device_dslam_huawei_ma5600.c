/*!
 * \file mdm_test_device_dslam_huawei_ma5600.c A test for dslams huawei ma5600.
 *
 * # ./mdm_test_device_dslam_huawei_ma5600 list
 *
...
#define	LOGFILE		1
#define	LOGFULL		1
#define	LOGPATH		"/tmp/mdm_test_device_dslam_huawei_ma5600.log"
...
main(int argc, char *argv[])
{
	...
	mdm_device_new(&device, MDM_DEVICE_DSLAM_HUAWEI_MA5600, &status);
	...
	if(strcmp(argv[1], "list") == 0)
	{
		...
		while((
			cmd_name = mdm_devicecmd_2stringname(MDM_DEVICE_DSLAM_HUAWEI_MA5600, i
		)) != NULL)
		{
			fprintf(
				stdout, "(%.2d) - !!%s!!%s!!\n",
				i, cmd_name,
				mdm_devicecmd_2string(MDM_DEVICE_DSLAM_HUAWEI_MA5600, i)
			);
			i++;
		}	
		goto done;
		...
		if(!mdm_devicecmd_isvalid(MDM_DEVICE_DSLAM_HUAWEI_MA5600, cmd))
		{
		...
		mdm_device_open(
			...
			sizeof(mdm_device_dslam_huawei_ma5600_options_t), 
			&status
		);
		if(status.status == MDM_OP_ERROR)
		{
