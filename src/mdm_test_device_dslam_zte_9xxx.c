/*!
 * \file mdm_test_device_dslam_zte_9xxx.c A test for dslams zte 9000 series.
 *
 * # ./mdm_test_device_dslam_zte_9xxx list
 *
...
main(int argc, char *argv[])
{
	...
	if(strcmp(argv[1], "list") == 0)
	{
		...
		while((
			cmd_name = mdm_devicecmd_2stringname(MDM_DEVICE_DSLAM_ZTE_9xxx, i
		)) != NULL)
		{
			fprintf(
				stdout, "(%.2d) - !!%s!!%s!!\n",
				i, cmd_name, mdm_devicecmd_2string(MDM_DEVICE_DSLAM_ZTE_9xxx, i)
			);
			i++;
		}	
		goto done;
		...
		if(!mdm_devicecmd_isvalid(MDM_DEVICE_DSLAM_ZTE_9xxx, cmd))
		{
		...
		mdm_device_open(
			...
			sizeof(mdm_device_dslam_zte_9xxx_options_t), 
			&status
		);
		if(status.status == MDM_OP_ERROR)
		{
